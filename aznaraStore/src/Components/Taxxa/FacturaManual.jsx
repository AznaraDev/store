...
const FacturaManual = () => {
  ...
  useEffect(() => {
    ...
    // No cleanup needed here, so just return nothing or undefined
    return undefined;
  }, [dispatch]);
      ...
      if (documentInput.length >= 6) {
        ...
        if (foundBuyer) {
          Swal.fire({
            ...
            text: `Comprador encontrado: ${foundBuyer.scostumername}`,
            icon: 'success',
            timer: 3000,
          ...
          Swal.fire({
            ...
            text: 'Comprador no encontrado. Por favor, complete los datos.',
            icon: 'info',
  ...
  const handleCreateBuyer = async () => {
    ...
    if (result && !result.error) {
      ...
      Swal.fire({
        title: '¡Éxito!',
        text: 'Comprador creado exitosamente. Ahora puede agregar items.',
        icon: 'success',
        timer: 3000,
        showConfirmButton: false
      });
  ...
  const handleSubmitInvoice = async () => {
    if (!buyer) {
      Swal.fire({
        ...
        text: 'Debe seleccionar o crear un comprador.',
        icon: 'error',
        confirmButtonText: 'Entendido'
      });
    ...
    if (validItems.length === 0) {
      Swal.fire({
        ...
        text: 'Agregue al menos un item válido.',
        icon: 'error',
        confirmButtonText: 'Entendido'
      });
