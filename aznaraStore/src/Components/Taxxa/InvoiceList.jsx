...
import { fetchAllInvoices, createCreditNote } from '../../Redux/Actions/taxxaActions';
...
const InvoiceList = () => {
  ...
  const { invoices = [], loading = false, error = null } = useSelector((state) => {
    return {
      invoices: state.taxxa.invoices?.data || [],        // ✅ Acceder a invoices.data
      loading: state.taxxa.invoices?.loading || false,   // ✅ Acceder al loading de invoices
      error: state.taxxa.invoices?.error || null         // ✅ Acceder al error de invoices
    };
  });
    ...
    try {
      ...
      if (retryAttempts < 3) {
        ...
        Swal.fire({
          ...
          text: 'Error al cargar las facturas después de varios intentos',
          icon: 'error',
  ...
  const handleCreateCreditNote = async () => {
    ...
    try {
      ...
      if (result.success) {
        ...
        dispatch(fetchAllInvoices()); // Recargar lista
      ...
      Swal.fire({
        title: 'Error',
        text: 'Error al crear la nota de crédito. Por favor, intenta nuevamente.',
        icon: 'error',
        confirmButtonText: 'Entendido'
      });
    }
