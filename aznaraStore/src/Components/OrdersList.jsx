    ...
    <div className="container mx-auto px-4 py-8">
      ...
      <div className="grid grid-cols-1 gap-4">
        {filteredOrders.map(order => (
          <div key={order.id_orderDetail} className={`border rounded p-4 ${order.state_order === 'Envío Realizado' ? 'bg-green-100' : ''}`}>
            <div className="flex justify-between items-center mb-2">
              <div className="font-semibold">Fecha: {order.date}</div>
              <div className="font-semibold bg-green-500 text-white">Estado Pedido: {order.state_order}</div>
            </div>
